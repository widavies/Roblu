...
public class TeamViewer extends AppCompatActivity implements ViewPager.OnPageChangeListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(readOnly) Utils.showSnackbar(findViewById(R.id.teams_viewer_layout), getApplicationContext(), "Read only mode is enabled when resolving conflicts", false, new Loader(getApplicationContext()).loadSettings().getRui().getPrimaryColor());
        ...
        final PopupMenu.OnMenuItemClickListener popupListener = new PopupMenu.OnMenuItemClickListener() {
            ...
            public boolean onMenuItemClick(MenuItem item) {
                if(item.getItemId() == R.id.match_page) {
                    ...
                    if(pager.getCurrentItem() == 0) Utils.showSnackbar(findViewById(R.id.teams_viewer_layout), getApplicationContext(), "Overiew can't be opened on TBA.", true, 0);
                ...
                if(item.getItemId() == R.id.delete_match) {
                    if(pager.getCurrentItem() == 0) Utils.showSnackbar(findViewById(R.id.teams_viewer_layout), getApplicationContext(), "Overiew can't be deleted", true, 0);
                ...
                if(item.getItemId() == won) {
                    if(pager.getCurrentItem() == 0) Utils.showSnackbar(findViewById(R.id.teams_viewer_layout), getApplicationContext(), "Overiew can't be marked as won", true, 0);
