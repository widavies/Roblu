...
import com.cpjd.roblu.sync.cloud.sync.InitPacker;
            ...
            if(preference.getKey().equals("edit_form")) {
                ...
                builder.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        Utils.showSnackbar(layout, getActivity(), "Duplicate event created", false, rui.getPrimaryColor());
                        dialog.dismiss();
                    }
                });
                ...
                if (preference.getKey().equals("backup")) {
                    if (EasyPermissions.hasPermissions(getActivity(), Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                        ...
                        Utils.showSnackbar(layout, getActivity(), "Storage permission is disabled. Please enable it.", true, rui.getPrimaryColor());
                    }
                    ...
                    builder.setPositiveButton("Delete", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            Utils.showSnackbar(layout, getActivity(), "Teams successfully deleted", false, rui.getPrimaryColor());
                            dialog.dismiss();
                        }
                    });
        ...
        public boolean onPreferenceChange(Preference preference, Object o) {
            if(preference.getKey().equals("sync")) {
                if(!new Loader(getActivity()).loadSettings().isSignedIn()) {
                    Utils.showSnackbar(layout, getActivity(), "You must sign in to Roblu Cloud in the settings before syncing an event.", true, 0);
                    return false;
                }
                if(!Utils.hasInternetConnection(getActivity())) {
                    Utils.showSnackbar(layout, getActivity(), "You are not connected to the internet.", true, 0);
                    return false;
                }
                if((boolean)o) {
                    ...
                    try {
                        ...
                        Utils.showSnackbar(layout, getActivity(), "Cloud sync disabled for "+event.getName(), false, rui.getPrimaryColor());
                        return true;
                    } catch(Exception e) {
            ...
            if(requestCode == Constants.FILE_CHOOSER) {
                try {
                    ...
                    if(os != null) {
                        ...
                        os.flush();
                        os.close();
                        Utils.showSnackbar(layout, getActivity(), "Successfully created backup file", false, rui.getPrimaryColor());
                        new Loader(getActivity()).cleanBackupDirs();
                    } else Utils.showSnackbar(layout, getActivity(), "Error occured while creating backup", true, 0);
                } catch(Exception e) {
                    Utils.showSnackbar(layout, getActivity(), "Error occured while creating backup", true, 0);
                }
            }
        ...
        public class BackupEvent extends AsyncTask<Void, Void, RBackup> {
            ...
            protected void onPostExecute(RBackup result) {
                ...
                try {
                    ...
                    } else Utils.showSnackbar(layout, getActivity(), "Your phone doesn't support backup exporting", true, 0);
                } catch(Exception e) {
                    Utils.showSnackbar(layout, getActivity(), "No file manager found", true, 0);
                }
            }
        }
    }
