...
import com.cpjd.roblu.ui.eventdrawer.DividerDrawerItem;
...
import com.cpjd.roblu.ui.events.CreateEventPicker;
import com.cpjd.roblu.ui.events.EventSettings;
...
public class TeamsView extends AppCompatActivity implements View.OnClickListener, TeamsItemClickListener, View.OnLongClickListener {
    ...
    private void createTeam() {
        ...
        Utils.setInputTextLayoutColor(rui.getAccent(), rui.getText(), null, input2);
        ...
        protected LinkedList<RTeam> doInBackground(Void... params) {
            ...
            if(!query.equals("")) {
                ...
                for(int i = 0; i < teams.size(); i++) {
                    ...
                    if(Utils.contains(name, query)) relevance += 400;
            ...
            try {
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Utils.showSnackbar(layout, getApplicationContext(), "Team files are corrupt.", true, rui.getPrimaryColor());
                    }
                });
                return null;
            }
    ...
    public void deleteTeam(RTeam team) {
        ...
        Utils.showSnackbar(layout, getApplicationContext(), team.getName()+" was successfully deleted", false, rui.getPrimaryColor());
        new LoadTeams(false, lastQuery, lastSortToken, lastFilter).execute();
    }
}